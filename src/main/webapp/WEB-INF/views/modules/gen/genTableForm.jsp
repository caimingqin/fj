						...
						<table id="contentTable" class="table table-striped table-bordered table-condensed">
							...
							<c:forEach items="${genTable.columnList}" var="column" varStatus="vs">
								<tr${column.delFlag eq '1'?' class="error" title="已删除的列，保存之后消失！"':''}>
									...
									<td>
										<input type="text" name="columnList[${vs.index}].javaField" value="${column.javaField}" maxlength="200" style="width:100px;" class="required input-small"/>
									</td>
									<td>
									...
									<td>
										<select name="columnList[${vs.index}].dictType" style="width:120px;">
										<option>--</option>
											<c:forEach items="${typeList}" var="dictType">
												<option value="${dictType}" ${dictType==column.dictType?'selected':''} >${dictType}</option>
											</c:forEach>
										</select>
									</td>
									<td>

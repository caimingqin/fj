...
<ul id="${input}Preview" style="padding:0;margin:0"></ul><div style="clear:both"></div><c:if test="${!readonly}"><a href="javascript:" onclick="${input}FinderOpen();" class="btn">${selectMultiple?'添加':'选择'}</a>&nbsp;<a href="javascript:" onclick="${input}DelAll();" class="btn">清除</a></c:if>
<script type="text/javascript">
	function ${input}FinderOpen(){//<c:if test="${type eq 'thumb'}"><c:set var="ctype" value="images"/></c:if><c:if test="${type ne 'thumb'}"><c:set var="ctype" value="${type}"/></c:if>
		...
		var url = "${ctxStatic}/ckfinder/ckfinder.html?type=${ctype}&action=js&func=${input}SelectAction&thumbFunc=${input}ThumbSelectAction&cb=${input}Callback&dts=${type eq 'thumb'?'1':'0'}&sm=${selectMultiple?1:0}";
		//设置定位到固定的文件夹start=${ctype}:${uploadPath}/"+year+"/"+month+
	...
	function ${input}SelectAction(fileUrl, data, allFiles){
		...
		for(var i=0; i<files.length; i++){//<c:if test="${type eq 'thumb'}">
			//url += decodeURIComponent(files[i].getThumbnailUrl());//</c:if><c:if test="${type ne 'thumb'}">
			url += decodeURIComponent(files[i].getUrl());//</c:if>
...
/* 	function ${input}ThumbSelectAction(fileUrl, data, allFiles){
	...
	function ${input}Preview(){
		...
		for (var i=0; i<urls.length; i++){
			if (urls[i]!=""){
				li += "<li style='list-style:none;float:left'><a href=\""+urls[i]+"\" url=\""+urls[i]+"\" target=\"_blank\"><img src=\""+urls[i]+"\" url=\""+urls[i]+"\" style=\"width:${empty maxWidth ? 200 : maxWidth}px;height:${empty maxHeight ? 200 : maxHeight}px;_height:${empty maxHeight ? 200 : maxHeight}px;border:0;padding:3px;\"></a></li>";
			}
		}
		
		console.log(li);
		$("#${input}Preview").append(li);
		console.log($("#${input}Preview").html());

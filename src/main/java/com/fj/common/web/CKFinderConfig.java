...
import com.fj.common.utils.FileUtils;
import com.fj.modules.sys.security.SystemAuthorizingRealm.Principal;
...
//			this.baseURL = ServletContextFactory.getServletContext().getContextPath()+"/userfiles/"+principal+"/";
			...
			String realPath=Global.getUserfilesBaseDir()+Global.USERFILES_BASE_URL+loginName+"/";
			FileUtils.createDirectory(FileUtils.path(realPath));
			this.baseURL = FileUtils.path(Servlets.getRequest().getContextPath() + Global.USERFILES_BASE_URL + loginName + "/");
			this.baseDir = FileUtils.path(Global.getUserfilesBaseDir() + Global.USERFILES_BASE_URL + loginName + "/");
		} catch (Exception e) {
